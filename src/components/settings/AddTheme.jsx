...
function AddTheme({ isOpen, onClose, updateData, onSubmit }) {
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            Object.keys(pickerRefs).forEach((key) => {
                if (
                    focused[key] &&
                    pickerRefs[key].current &&
                    !pickerRefs[key].current.contains(e.target)
                ) {
    ...
    const renderPicker = (field, label) => (
        <div className="flex flex-col">
            <label className="font-semibold">{label}</label>
            <div className="relative" ref={pickerRefs[field]}>
                <span
                    tabIndex={0}
                    style={{ backgroundColor: data?.[field] }}
                    onClick={() => setFocused({ ...focused, [field]: true })}
                ...
                {focused[field] && (
                    <div className="absolute top-full left-0 mt-2 z-50">
                        <HexColorPicker
                            color={data?.[field]}
                            onChange={(c) => handleFieldChange(field, c)}
                        />
                    </div>
                )}
            </div>
        ...
        >
            <div className="bg-bg p-4 rounded border border-bbg w-72 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                    {renderPicker("bg", "Background Color")}
                    {renderPicker("fg", "Text Color")}
                    {renderPicker("btnbg", "Button BG Color")}
                    {renderPicker("bbg", "Border Color")}
                    <div className="flex flex-col col-span-2">
                        <label className="font-semibold">My Identity(Logo duh)</label>
                        <div className="relative" ref={pickerRefs.logo}>
                            {showLColor ? (
                                <>
                                    <span
                                        tabIndex={0}
                                ...
                                <div className="flex text-[11px] text-bbg justify-between items-center">
                                    ...
                                    <div className="flex space-x-2">
                                        <button
                                            onClick={handleYeah}
                                            className="hover:underline hover:text-btnbg transition duration-100 border-none"
                                        >
                                            Yeah
                                        </button>
                                        <button
